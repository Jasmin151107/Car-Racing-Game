  ...
  if(gameState===PLAY){
   ...
   path.velocityX = -(6 + 2*distance/150);
    ...
    if(yellowCG.isTouching(car)){
      gameState = END;
      player1.velocityY = 0;
      player1.addImage("opponentPlayer1",oppYellowImg);
     }
     if(orangeCG.isTouching(car)){
      gameState = END;
      player2.velocityY = 0;
      player2.addImage("opponentPlayer2",oppOrangeImg);
    }
    if(blueCG.isTouching(car)){
      gameState = END;
      player3.velocityY = 0;
      player3.addImage("opponentPlayer3",oppBlueImg);
    }
  }
...
function reset(){
  gameState = PLAY;
  gameOver.visible = false;
  car.addImage ("SahilRunning",carImg);
